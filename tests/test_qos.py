...
class TestQosAPI:
    ...
    def test_apply_quality_score(self, client, auth_headers, reservation_id):
        resp = client.post(
            f"/api/v1/interface/{reservation_id}/qos",
            headers=auth_headers,
            json={"download_quality": 80},
        )
        assert resp.status_code == 200
        data = resp.json()
        assert data["download_quality"] == 80
        assert data["download_netem_params"] is not None
        assert data["download_netem_params"]["packet_loss_percent"] > 0

    def test_apply_quality_and_speed(self, client, auth_headers, reservation_id):
        resp = client.post(
            f"/api/v1/interface/{reservation_id}/qos",
            headers=auth_headers,
            json={"download_speed_kbit": 8000, "download_quality": 80, "upload_quality": 65},
        )
        assert resp.status_code == 200
        data = resp.json()
        assert data["download_speed_kbit"] == 8000
        assert data["download_quality"] == 80
        assert data["upload_quality"] == 65
        assert data["download_netem_params"] is not None
        assert data["upload_netem_params"] is not None

    def test_apply_advanced_override(self, client, auth_headers, reservation_id):
        resp = client.post(
            f"/api/v1/interface/{reservation_id}/qos",
            headers=auth_headers,
            json={
        ...
        data = resp.json()
        assert data["upload_netem_params"] is not None
        assert data["upload_netem_params"]["packet_loss_percent"] == 5.5
        assert data["upload_netem_params"]["delay_distribution"] == "paretonormal"

    def test_null_resets_quality(self, client, auth_headers, reservation_id):
        client.post(
            f"/api/v1/interface/{reservation_id}/qos",
            headers=auth_headers,
            json={"download_quality": 80, "download_speed_kbit": 8000},
        )
        resp = client.post(
            f"/api/v1/interface/{reservation_id}/qos",
            headers=auth_headers,
            json={"download_quality": None},
        )
        assert resp.status_code == 200
        data = resp.json()
        assert data["download_quality"] is None
        assert data["download_netem_params"] is None
        assert data["download_speed_kbit"] == 8000  # preserved

    def test_invalid_quality_range(self, client, auth_headers, reservation_id):
        resp = client.post(
            f"/api/v1/interface/{reservation_id}/qos",
            headers=auth_headers,
            json={"download_quality": 101},
        )
        assert resp.status_code == 422
